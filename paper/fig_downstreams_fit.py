...
def compute_J_and_integrand(
    ...
    *,
    ...
    dU = np.fft.rfft(U0 - U1)
    dU2 = np.abs(dU) ** 2

    if noise.model == "gaussian":
        resid = y - mu_hat
        S = estimate_gaussian_noise_psd(resid)
        integrand = (p**2) * A2 * dU2 / S
...
def plot_intro_figure(
    ...
    *,
    ...
    title: str = "",
    f_max: float = 0.5,
    t_start: str = "2020-09-01",
    t_end: str = "2021-03-31",
    show: bool = True,
):
    """
    ...
    #
    # IMPORTANT:
    # Preserve the previous unnormalized FFT definition.
    # No division by T.
    # ------------------------------------------------------------
    Tw = len(U0_w)
    ...
    if noise.model == "gaussian":
        ...
        noise_level = float(Tw * sigma ** 2)
        noise_label = "Noise variance level"
        ...
        noise_level = float(Tw * mmax)
        noise_label = r"Noise level $m_{\max}$"
        ...
        # Consistent with raw |rFFT|^2

        noise_level = float(Tw * nb_variance_level)
        noise_label = "Noise variance level"
    ...
    axC.ticklabel_format(
        axis="y",
        style="sci",
        scilimits=(0, 0),
    )
    axC.yaxis.get_offset_text().set_size(8)
...
if __name__ == "__main__":
    ...
    # test_sd = emp_sd
    #
    # g = lognormal_pmf_from_mean_sd(mean_days=test_mean, sd_days=test_sd, tau_max=60)
